...
#define NEUTRON 1        // tipo de particula (para graficar con VND)
#define PROTON  2        // tipo de particula (para graficar con VND)
...
#define D       0.0      // potential parameter (207)
...
double inicial(double *x,double *v,double *p,double *f,int *ptype,double rho,double temp,double xproton,int ndim)
{
 int    h,i,j,k,n,np,nn,nless,nplus;
...
void get_table_pandha(double *data,int *ptype,double r2,int ii,int jj,int ntable)
{
  ...
  if (itype!=jtype)  mytable=table_np;
  else if (itype==1) mytable=table_nn;
       else mytable=table_pp;
