...
FixTFMCPAULI::FixTFMCPAULI(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  xd(NULL), rotflag(0), random_num(NULL)
{
  if (narg < 6) error->all(FLERR,"Illegal fix tfmc-Pauli command");
  ...
  if (d_max <= 0) error->all(FLERR,"Fix tfmc-Pauli displacement length must be > 0");
  if (T_set <= 0) error->all(FLERR,"Fix tfmc-Pauli temperature must be > 0");
  if (seed <= 0) error->all(FLERR,"Illegal fix tfmc-Pauli random seed");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"com") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix tfmc-Pauli command");
      comflag = 1;
      ...
      if (iarg+1 > narg) error->all(FLERR,"Illegal fix tfmc-Pauli command");
      rotflag = 1;
      iarg += 1;
    } else error->all(FLERR,"Illegal fix tfmc-Pauli command");
  }
  ...
  if (comflag)
    if (xflag < 0 || xflag > 1 || yflag < 0 || yflag > 1 ||
        zflag < 0 || zflag > 1)
      error->all(FLERR,"Illegal fix tfmc-Pauli command");
...
void FixTFMCPAULI::init()
{
  ...
  if (has_shake > 0)
    error->all(FLERR,"Fix tfmc-Pauli is not compatible with fix shake");
...
void FixTFMCPAULI::initial_integrate(int vflag)
{
  double boltz = force->boltz;
  double **x = atom->x;
  double **v = atom->v;                        // Guillermo Frank: input velocity 
  double **f = atom->f; 
  double **mu = atom->mu;                      // Guillermo Frank: input momentum
  double *rmass = atom->rmass;
  double *mass = atom->mass;
  double massone;
  double masstotal;
  double xcm_d[3], xcm_dall[3];
  double d_i, xi, pi;                          // Guillermo Frank: define pi;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      // velocities (no forces) - Guillermo Frank -------------------

      for (int j = 0; j < 3; j++) {
        P_acc = 0.0;
        P_ran = 1.0;
        gamma =  -v[i][j] * d_i  / (2.0*boltz*T_set);
        gamma_exp = exp(gamma);
        gamma_expi = 1.0/gamma_exp;
        // generate displacements according to the tfMC distribution
        while (P_acc < P_ran) {
          pi = 2.0*random_num->uniform() - 1.0;
          P_ran = random_num->uniform();
          if (pi < 0) {
